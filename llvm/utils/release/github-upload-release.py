...
def create_release(repo, release, tag=None, name=None, message=None):
    ...
    if not message:
        message = dedent(
            ...
            Volunteers make binaries for the Arkari project, which will be uploaded
...
if args.user:
    ...
    if user.login != gh.get_user().login:
        print("User {} is not allowed to modify releases".format(args.user))
        sys.exit(1)
...
if args.command == "create":
    create_release(arkari_repo, args.release)
if args.command == "upload":
    upload_files(arkari_repo, args.release, args.files)
